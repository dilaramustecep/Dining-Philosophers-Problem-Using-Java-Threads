...
public class Philosopher extends Thread {
	...
	public static void main(String args[]) {
		...
		Semaphore barr=new Semaphore(4);
	...
	public void Take_Forks(int i){
		System.out.print(id+" tries to pick fork \n");

		/* IMPLEMENTATION */
		try {

			mutex.acquire(); 				// critical region
			state[this.id]=HUNGRY;			// set philosopher state to common array
			this.Test(i); 					// try to get forks either left or right one
	...
	public void Test(int i){
		...
		// if i hungry and there are avaliable forks etiher on left or right
		// it can eat
		if (state[i]== HUNGRY && state[left]!=EATING && state[right]!= EATING){
			System.out.print(i +" is eating \n");
			state[i]=EATING;	// change philosopher state
	...
	public void Put_Forks(int i){
		System.out.print(id+" tries to put forks \n");

		try { //down is acquire, up is release in our context
			mutex.acquire(); 			// critical region
			state[this.id]=THINKING;	// philosopher is full back to thinking
			table.ForkPut_GUI(i);
			table.StopEating_GUI(i);

			// YET CHECK NEIGHBOUR CAN EAT OTHERWISE IT CAUSES A PROBLEM
			int left=i-1;	int right=i+1;
	...
	public void run() {
		...
		// 1.Philosopher walks into table random amount of time (1 - 10 sec)
		...
		// 2.Philosopher sits to the table
		table.PutPlate_GUI(this.id);


		// 3. Philosopher waits other philosophers to sit IMPLEMENT BARRIER
		try {

			mutex.acquire();
			nDone=nDone+1;
			System.out.print(this.id+" is id and nDone is "+nDone+"\n");
			mutex.release();
		...
		System.out.print(this.id+"start to dining\n");
		...
		while (true){
			...
			// 4. Philosopher thinks random amount of time
			System.out.print(this.id+" started to think \n");
			...
			// 5. Philosopher gets hungry
			System.out.print(this.id+" gets hungry \n");
			table.Hungry_GUI(this.id);

			// 6. Philosopher takes forks
			this.Take_Forks(this.id);

			// 7. Philosopher eats

			// 8. Philosopher puts forks
			this.Put_Forks(this.id);
...
/*
* When arriving table maybe we need state related assignment check later on
*
* */